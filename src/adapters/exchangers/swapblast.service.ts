...
export class SwapBlastService {
    ...
    async getPoolsData(): Promise<PoolData[]> {
        const rateData = await fetch("https://raw.githubusercontent.com/SwapBlasted/swapblast-lists/main/swapblast.farmslist.json", {
            method: "GET",
            headers: {
        ...
        const response = fetch(this.BASE_GRAPHQL_URL, {
            ...
            .then(async (data): Promise<PoolData[]> => {
                ...
                if (rewardTokenPair) {
                    rewardTokenUsdPrice = new BN(rewardTokenPair?.reserveUSD)
                        .div(2)
                        .div(rewardTokenPair?.reserve1)
                        .toFixed(6)
                }
