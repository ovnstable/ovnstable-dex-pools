...
import { isCommunityResource } from '@ethersproject/providers/lib/formatter';
...
export class FallbackProvider extends BaseProvider {
  ...
  async detectNetwork(): Promise<Network> {
    // If RPC provider return error (for example: limit credit is down) then we ignored this network;
    let networks = await Promise.all(
      this.providerConfigs.map((c) =>
        c.provider.getNetwork().catch((reason) => {
          c.provider;
          return reason;
        }),
      ),
    ...
    networks = networks.filter((value) => !(value instanceof Error));
  ...
  async perform(method: string, params: { [name: string]: any }): Promise<any> {
    ...
    for (const config of configs) {
      try {
        return await getRunner(config, null, method, params);
      } catch (error) {
        console.log(`Provider [${config.id}] ${error}`);
        config.done = true;
        config.error = error;
      }
    }

    const e = <any>configs[0].error;
    ...
    logger.throwError(e.reason || e.message, code, props);
  }
}
