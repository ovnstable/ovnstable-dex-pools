...
export class SwapBlastService {
    ...
    async getPoolsData(): Promise<PoolData[]> {
        ...
        const response = fetch(this.BASE_GRAPHQL_URL, {
            ...
            .then(async (data): Promise<PoolData[]> => {
                ...
                console.log(responseBody.data.pairs, '---responseBody.data.pairs')
                let itemCount = 0;
                ...
                try {
                    ...
                    this.logger.error("Error when load apr for " + ExchangerType.SWAPBLAST);
                    return pools;
                }
            })
            .catch((e) => {
    ...
    private async initAprs(ovnPools: PoolData[]): Promise<PoolData[]> {
        ...
        this.logger.debug("Browser is start. " + ExchangerType.SWAPBLAST);
        ...
        try {
            ...
            const errorMessage = `Error when load ${ExchangerType.SWAPBLAST} pairs. url: ${url}`;
            this.logger.error(errorMessage, e);
            throw new ExchangerRequestError(errorMessage);
        } finally {
            this.logger.debug("Browser is close. " + ExchangerType.SWAPBLAST);
            await browser.close();
        }
